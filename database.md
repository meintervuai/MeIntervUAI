...
|---|---|---|
| `sesi_wawancara` | FR-09,10,22 | id, profil_id FK, mode (`teks/audio/video`), bahasa, posisi_target, perusahaan_target (teks, nullable), status (`berlangsung/selesai/dibatalkan`), skor_total, predikat, level, tanggal_mulai, tanggal_selesai |
...
| `evaluasi_sesi` / `evaluasi_wawancara` | FR-12,13,22 | id, sesi_id FK, profil_id FK, skor_total, skor_verbal, skor_non_verbal, predikat, metrik jsonb (`relevansiIsi`, `strukturStar`, `kosaKataProfesional`, `kepercayaanDiri`), analisis_non_verbal jsonb (`kontakMataPersen`, `posturTegakPersen`, `skorKepercayaanDiri`, `predikatNonVerbal` via MediaPipe Vision AI), kekuatan jsonb, area_peningkatan jsonb, rincian_pertanyaan jsonb, tanggal timestamptz. Cadangan offline: `localStorage` (`mentervu_riwayat_simulasi`). |
...
| 007 | `007_rekomendasi_lowongan.sql` | M5 | Tambah `perusahaan_target` pada `sesi_wawancara`; buat tabel `lowongan`, `rekomendasi_lowongan`, RLS, dan indeks |
| 008+ | (fase berikut) | M4–M5 | `metrik_performa`, `saran_revisi_cv`, `notifikasi`, dll |
